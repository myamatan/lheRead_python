...
inputDire='input_JHU_es/'
outputDire='parton_JHU_es/'
...
if __name__ == "__main__":
    ...
    tr.Branch('Mlljj', mZZ, 'Mlljj/F')
    tr.Branch('Ptlljj', PtZZ, 'Ptlljj/F')
    tr.Branch('Ptll', Ptll, 'Ptll/F')
    tr.Branch('Ptjj', Ptqq, 'Ptjj/F')
    tr.Branch('Ptratio', Ptratio, 'Ptratio/F')
    ...
    for e in pylhe.readLHE (inputDire+args[1]+'.lhe'):
        ...
        Ptll[0] = (l1 + l2 ).Pt()
        Ptqq[0] = (q1 + q2).Pt()
        Ptratio[0] = np.sqrt((l1+l2).Pt()**2 + (q1+q2).Pt()**2)/(l1 + l2 + q1 + q2).M()
